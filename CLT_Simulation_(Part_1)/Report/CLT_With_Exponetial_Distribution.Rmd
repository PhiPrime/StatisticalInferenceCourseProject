...
The theoretical mean of an exponetial distribution is $\frac{1}{\lambda}$. In our simulation we set $\lambda =$ `r l`. As such the theoretical mean is $\mu_T$ = `r 1/l`. We'll now look at how the distribution of the sample means compares to $\mu_T$  
...
In this histogram of the means the blue line represents $\mu_T$, `r 1/l` and the orange line represents the mean of the sample means, `r round(mean(means), 2)`. This shows that the mean of `r sims` simulations of `r n` samples from an exponetial distribution closely resembles the true theoretical mean.  
